...
(function() {
  ...
  console.log("CurbYourCarbon: Universal tracker initialized on", window.location.hostname);
  ...
  const PERIODIC_INTERVAL = 60 * 1000;
  const periodicSummary = setInterval(() => {
    const timeActive = state.activeTime.getActiveMinutes();
    const timeSinceLastSend = (Date.now() - state.lastSendTime) / 60000;
    
    // Send if we have 45+ seconds of activity OR 60 seconds have passed
    if (timeActive >= 0.75 || timeSinceLastSend >= 0.95) {
